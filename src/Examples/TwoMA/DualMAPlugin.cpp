...
void DualMAPlugin_onOrderFilled(QMSServer &svr, const qms_barData &bd, const void * pExtra);

void DualMAPlugin_onOrderError(QMSServer &svr, const qms_barData &bd, const void * pExtra);

void DualMAPlugin_onLimitPlaced(QMSServer &svr, const qms_barData &bd, const void * pExtra);

void DualMAPlugin_onLimitCancelled(QMSServer &svr, const qms_barData &bd, const void * pExtra);

void DualMAPlugin_onLimitError(QMSServer &svr, const qms_barData &bd, const void * pExtra);

void DualMAPlugin_onLimitEdited(QMSServer &svr, const qms_barData &bd, const void * pExtra);

void DualMAPlugin(QMSServer &svr, int msg, const qms_barData &bd, const void * pExtra)
{
	switch(msg) {
	case eQMSQuantMsg_tick: return DualMAPlugin_onTick(svr, bd);
	case eQMSQuantMsg_init: return DualMAPlugin_onInit(svr, bd);
	case eQMSQuantMsg_shutdown: return DualMAPlugin_onShutdown(svr, bd);

	case eQMSQuantMsg_onOrderFilled:return DualMAPlugin_onOrderFilled(svr, bd, pExtra);
	case eQMSQuantMsg_onOrderError:return DualMAPlugin_onOrderError(svr, bd, pExtra);
	case eQMSQuantMsg_onLimitPlaced:return DualMAPlugin_onLimitPlaced(svr, bd, pExtra);
	case eQMSQuantMsg_onLimitError:return DualMAPlugin_onLimitError(svr, bd, pExtra);
	case eQMSQuantMsg_onLimitCancelled:return DualMAPlugin_onLimitCancelled(svr, bd, pExtra);
	case eQMSQuantMsg_onLimitEdited:return DualMAPlugin_onLimitEdited(svr, bd, pExtra);
	default:
		return;
	}
}
...
void DualMAPlugin_onTick(QMSServer &svr, const qms_barData &bd)
{
	...
	if(!bd.ptr || !bd.acct || !bd.symbol || !bd.pExposure) {
		svr.Q_UpdateStatusMsg(who, "\2""\xc""INVALID PARAMETERS");
		return;
	}
	...
	int slowBeg = 0;
	int slowCount = 0;
	r = TA_MA(0, count - 1, in, slowPeriod, (TA_MAType)slowMA, &slowBeg,&slowCount, outSlow);
	if(fastCount < 4) {
		return;
	}
	...
	ss << "time=" << bd.price.tim << "Count=" << bd.count << "\n";
	...
	if(-1 > signal) {
		...
		if(1E-6 < fabs(expVol)) {
			if(0 < expVol) {
				svr.Q_PlaceMarketOrder(who, ida, ids, oid.x64, -expVol, 0, "CLOSE_EXPOSURE");
				++oid;
		...
		if (qs.useLimits)
		{
			...
			price += pips;
			r = svr.Q_PlaceLimit(who, ida, ids, oid.x64, 2, 0, vol, price, 0, "SELL_LIMIT");
			ss << "Placed sell limit: " << oid.x64 << " : vol = " << vol << ", result = " << r << "\n";
		} else {
			price;
			r = svr.Q_PlaceMarketOrder(who, ida, ids, oid.x64, vol, 0, "SELL_MARKET");
			ss << "Placed Market order: " << oid.x64 << " : vol = " << vol << ", result = " << r << "\n";
		}
		...
		if(qs.useTakeProfit) {
			...
			double x = price - pips;
			qs.tpId = svr.Q_PlaceLimit(who, ida, ids, oid.x64, 2, 9, -vol, x, 0, "SELL_TP");
			qs.tpOrderId = oid.x64;
			ss << "Placed Sell Take Profit Sell limit: " << oid.x64 << " : vol = " << -vol << ", price = " << x << "\n";
		}
		...
		if(qs.useStopLoss) {
			...
			double x = price + pips;
			qs.slId = svr.Q_PlaceLimit(who, ida, ids, oid.x64, 3, 9, -vol, x, x, "SELL_SL");
			qs.slOrderId = oid.x64;
			qs.slSide = -1;
			qs.slLastUpdatePrice = x;
			qs.slPips = pips;
			ss << "Placed Sell Stop loss: Sell Stop: " << oid.x64 << " : vol = " << -vol << ", price = " << x << "\n";
		}
	...
	if(1 < signal) {
		...
		if (qs.useLimits)
		{
			double pips = qs.td * onePip;
			price -= pips;
			r = svr.Q_PlaceLimit(who, ida, ids, oid.x64, 2, 9, vol, price, 0, "BUY_LIMIT");
			ss << "Placed buy limit: " << oid.x64 << " : vol = " << vol << ", result = " << r << "\n";
		} else {
			r = svr.Q_PlaceMarketOrder(who, ida, ids, oid.x64, vol, 0, "BUY_MARKET");
			ss << "Placed Sell Market: " << oid.x64 << " : vol = " << vol << ", price = " << price << "\n";
		}
		...
		if(qs.useTakeProfit)
		{
			...
			double x = price + pips;
			++oid;
			qs.tpId = svr.Q_PlaceLimit(who, ida, ids, oid.x64, 2, 9, -vol, x, 0, "BUY_TP");
			ss << "Placed Buy Take Profit Sell limit: " << oid.x64 << " : vol = " << -vol << ", price = " << x << "\n";
			qs.tpOrderId = oid.x64;
		}
		...
		if (qs.useStopLoss)
		{
			...
			double x = price - pips;
			++oid;
			qs.slId = svr.Q_PlaceLimit(who, ida, ids, oid.x64, 3, 9, -vol, x, x, "BUY_SL");
			qs.slOrderId = oid.x64;
			qs.slSide = 1;
			qs.slLastUpdatePrice = x;
			qs.slPips = pips;
...
void DualMAPlugin_onInit(QMSServer &svr, const qms_barData &bd)
{
	...
	ss << "Started TWOMA: " << bd.symbol->name << ", acct=" << bd.acct->name << "(" << bd.acct->id << "): ";
