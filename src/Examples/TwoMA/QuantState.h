...
struct QuantState
{
	...
	char useStopLoss;   // Set to 1  to use stop loss. 0 if no stop loss.
	char useTakeProfit; // Set to 1 to use take profit, 1 if no take profit.
	char dummy;         // Not used, for alignment.
	double td;
	double tp;
	double sl;

	double volume;    // Volume to trade.
	int fastMA;       // Fast moving average type
	int fastMAPeriod; // Fast moving average period.
	int slowMA;       // Slow moving average type
