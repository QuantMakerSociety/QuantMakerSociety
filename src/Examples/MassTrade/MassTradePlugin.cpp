...
void MassTrade_onTick(QMSServer &svr, const qms_barData &bd)
{
	...
	int nOrders = cfg[eCfgItem_nOrdersPerTick].val.i32;

	if(1E-6 > td || 1 > nOrders) {
		td = 0;
	}
	...
	if(1E-6 < volume) {
		int ms = cfg[eCfgITem_timeBetweenOrders].val.i32;
		if(0 < ms) {
			int64_t ct = GetTickCount64();
			int64_t delta = ct - qs.lastMS;
			if(delta < ms) {
				return;
			}
			qs.lastMS = ct;
